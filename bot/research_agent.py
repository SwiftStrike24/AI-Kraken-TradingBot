...
class ResearchAgent:
    ...
    def __init__(self, logs_dir: str = "logs"):
        ...
        try:
            self.openai_client = OpenAI(max_retries=2)
            logger.info("OpenAI client initialized for market analysis with max_retries=2")
        ...
        for feed_url in feed_urls:
            try:
                ...
                with requests.Session() as s:
                    ...
                    response = s.get(feed_url, timeout=20)
...
Deliver as professional, flowing analysis text suitable for institutional morning briefing."""
    ...
    def generate_daily_report(self, coingecko_data: Dict[str, Any] = None, custom_query: Optional[str] = None, bypass_cache: bool = False, reflection: Optional[Dict[str, Any]] = None, reflection_model: str = "gemini-2.5-pro") -> str:
        """
        ...
        Args:
            ...
            reflection: Optional reflection report dict generated by ReflectionAgent (Gemini).
            reflection_model: Name of the model used to generate reflection (default gemini-2.5-pro).
        ...
        try:
            ...
            # Append historical reflection section (Gemini) if available
            try:
                if reflection and isinstance(reflection, dict):
                    report_sections.append("**🧠 Historical Reflection:**")
                    report_sections.append(f"_Model: {reflection_model}_")
                    report_sections.append("")
                    summary_text = reflection.get("summary", "No reflection summary available.")
                    ...
                    if isinstance(key_learnings, list) and key_learnings:
                        report_sections.append("Key Learnings:")
                        for item in key_learnings[:6]:
                            report_sections.append(f"- {str(item)}")
                        report_sections.append("")
                    focus = reflection.get("recommended_focus")
                    if focus:
                        report_sections.append(f"Recommended Focus: {focus}")
                        report_sections.append("")
            except Exception as e:
                logger.warning(f"Could not append reflection section: {e}")
